...
function initializeButtons() {
    document.getElementById('stop-tpms').addEventListener('click', stopTPMSCollection);
    ...
    for (let i = 1; i <= tireCount; i++) {
        ...
        dataBox.innerHTML = `
            ...
            <div class="details">
                ...
                <div class="detail-item">
                    <span>Battery:</span>
                    <span id="data-box-battery-${i}">${tire.battery.toFixed(0)}%</span>
                </div>
            </div>
        `;
...
function updateDataBox(tireNum) {
    ...
    dataBox.className = `data-box ${tire.status}`;
    document.getElementById(`data-box-pressure-${tireNum}`).textContent = `${tire.pressure.toFixed(1)} PSI`;
    document.getElementById(`data-box-temperature-${tireNum}`).textContent = `${tire.temperature.toFixed(1)} °C`;
    document.getElementById(`data-box-battery-${tireNum}`).textContent = `${tire.battery.toFixed(0)}%`;
}
...
function calculateTirePositions(axleConfig) {
    ...
    for (let i = 0; i < numAxles; i++) {
        ...
        const sideHeight = 13; // Vertical gap between tires on the same side
